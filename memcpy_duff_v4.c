...
void *memcpy_duff_v4(void *__restrict dst, const void *__restrict src, size_t size)
{
	v4 *vdst;
	const v4 *vsrc;
...
int main(int argc, char **argv)
{
	...
	for (i = 0; i < lim; i++) {
		...
		/* Warm-up. */
		memcpy_duff_v4(mem1 + 1, mem, mem_size - 1);
		assert(memcmp(mem1 + 1, mem, mem_size - 1) == 0);
		memcpy_duff_v4(mem, mem1, mem_size);
		assert(memcmp(mem, mem1, mem_size) == 0);
	}

	clock_gettime(CLOCK_MONOTONIC, &start);
	memcpy_duff_v4(mem1, mem, mem_size);
